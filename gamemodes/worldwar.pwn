...
// Pawno libs
#include "./includes/lottery.pwn"
#include "./includes/Passenger.pwn"
#include "./includes/Events/fallout.pwn"
#include "./includes/Care Package.pwn"

//discord enums
new DCC_Channel:g_Discord_Chat;
new DCC_Channel:logs;
//new DCC_Channel:g_Discord_join;
...
new const gNotifications[][] =
{
    ...
    "We also have our forums/website where you will find intersting discussions and game relate talks, visit ~b~~h~~h~~h~http://COMING SOON",
	...
	"Use /updates to check the latest updates which is added to the server"
};
new Text:txtNotify;
new gNotifyId;
new gNotifyGap;
...
{
    //Europe Base
	CA_RemoveBuilding(8158, 1727.8047, 1018.6328, 14.8203, 0.25);
	CA_RemoveBuilding(1215, 1707.3359, 990.5234, 10.2891, 0.25);
	CA_RemoveBuilding(1365, 1745.0703, 987.1328, 10.9609, 0.25);
	CA_RemoveBuilding(8061, 1727.8047, 1018.6328, 14.8203, 0.25);
	CA_RemoveBuilding(1616, 1708.0234, 1049.0469, 16.0781, 0.25);
	...
	CA_RemoveBuilding(1345, -218.0313, 1164.9219, 19.5391, 0.25);
	...
	CA_RemoveBuilding(3539, 1030.7891, 2318.2266, 12.3438, 0.25);
	CA_RemoveBuilding(3537, 982.4219, 2316.7969, 12.3438, 0.25);
	CA_RemoveBuilding(3536, 982.3203, 2343.5469, 12.1250, 0.25);
	CA_RemoveBuilding(3444, 983.8281, 2268.1953, 12.3438, 0.25);
	CA_RemoveBuilding(3464, 957.0781, 2268.1250, 12.1250, 0.25);
	CA_RemoveBuilding(3460, 940.3828, 2297.8203, 13.7656, 0.25);
	CA_RemoveBuilding(1350, 917.6641, 2298.5313, 9.7813, 0.25);
	CA_RemoveBuilding(3444, 982.4219, 2316.7969, 12.3438, 0.25);
	CA_RemoveBuilding(3464, 982.3203, 2343.5469, 12.1250, 0.25);
	CA_RemoveBuilding(3466, 1030.7891, 2318.2266, 12.3438, 0.25);
	...
	CA_RemoveBuilding(11437, -775.5938, 1555.6797, 26.0938, 0.25);
	...
	//Broken Bridge
	CA_RemoveBuilding(10822, -1336.8125, 892.7578, 57.9141, 0.25);
	CA_RemoveBuilding(11251, -1336.8125, 892.7578, 57.9141, 0.25);
	CA_RemoveBuilding(11381, -1349.3281, 879.4219, 43.0313, 0.25);
	CA_RemoveBuilding(1290, -1397.7344, 827.5391, 52.7891, 0.25);
	CA_RemoveBuilding(1290, -1331.1328, 897.4063, 51.8125, 0.25);
	CA_RemoveBuilding(1290, -1264.5703, 967.3438, 50.1875, 0.25);
	...
	txtConnect[0] = TextDrawCreate(312.000000, 302.000000, "Mortal Kombat");
	...
	{
	    gTeam[i][teamBaseId] = GangZoneCreate(gTeam[i][teamBase][0], gTeam[i][teamBase][1], gTeam[i][teamBase][2], gTeam[i][teamBase][3]);
    	GangZoneBorderCreate(gTeam[i][teamBaseBorderId], gTeam[i][teamBase][0], gTeam[i][teamBase][1], gTeam[i][teamBase][2], gTeam[i][teamBase][3]);
    	gTeam[i][teamBaseAreaId] = CreateDynamicRectangle(gTeam[i][teamBase][0], gTeam[i][teamBase][1], gTeam[i][teamBase][2], gTeam[i][teamBase][3]);
	...
	for (new i, j = sizeof(gZone); i < j; i++)
	{
	    gZone[i][zoneId] = GangZoneCreate(gZone[i][zonePos][0], gZone[i][zonePos][1], gZone[i][zonePos][2], gZone[i][zonePos][3]);
	   	GangZoneBorderCreate(gZone[i][zoneBorderId], gZone[i][zonePos][0], gZone[i][zonePos][1], gZone[i][zonePos][2], gZone[i][zonePos][3]);
	   	for (new x; x < 10; x++)
	   	{
	   	    gZone[i][zoneNumberId][x] = -1;
	   	}
	   	GangZoneNumberCreate(gZone[i][zoneNumberId], gZone[i][zonePos][0], gZone[i][zonePos][1], gZone[i][zonePos][2], gZone[i][zonePos][3], i);
	...
	g_Discord_Chat = DCC_FindChannelById("808716917022916638");
	logs = DCC_FindChannelById("808717733335924767");
//	g_Discord_join = DCC_FindChannelById("808716917022916638"); // Discord channel ID
...
public OnPlayerConnect(playerid)
{
	...
	for (new i; i < MAX_TEAMS; i++)
	{
	    GangZoneShowForPlayer(playerid, gTeam[i][teamBaseId], SET_ALPHA(gTeam[i][teamColor], 100));
	    for (new x; x < 4; x++)
		{
			GangZoneShowForPlayer(playerid, gTeam[i][teamBaseBorderId][x], SET_ALPHA(COLOR_RED, 150));
		}
	}

	for(new i, j = sizeof(gZone); i < j; i++)
	{
		GangZoneShowForPlayer(playerid, gZone[i][zoneId], SET_ALPHA(gTeam[gZone[i][zoneOwner]][teamColor], 100));
	    for (new x; x < 4; x++)
		{
			GangZoneShowForPlayer(playerid, gZone[i][zoneBorderId][x], SET_ALPHA(COLOR_BLACK, 150));
		}
	    for (new x; x < 10; x++)
		{
			GangZoneShowForPlayer(playerid, gZone[i][zoneNumberId][x], SET_ALPHA(COLOR_BLACK, 150));
		}
	}
...
public OnPlayerSpawn(playerid)
{
	...
	if (! pLogged[playerid])
	{
		...
		if (rowid == SQL_INVALID_ROW)
		{
		    ...
		    SendClientMessage(playerid, COLOR_GREEN, "Welcome to Mortal Kombat, SAMP gaming community.");
		...
		else
		{
		    ...
		    SendClientMessage(playerid, COLOR_GREEN, "Welcome back to Mortal Kombat, SAMP gaming community.");
	...
	switch (dialogid)
	{
	    case DIALOG_ID_REGISTER:
	    {
			if (response)
			{
			    ...
			    SendClientMessage(playerid, COLOR_GREEN, "Great job! now you are an official member Mortal Kombat community.");
	    ...
	    case DIALOG_ID_HELP:
	    {
	        if (response)
		    {
				switch (listitem)
				{
					...
					case 1:
					{
						strcat(info, ""WHITE"Server Owner: "SAMP_BLUE"Bo$$\n\n");
						strcat(info, "Please visit and register on our website "SAMP_BLUE"COMING SOON"WHITE".\n\n");
						strcat(info, ""LIME"Credits:\n");
						strcat(info, ""WHITE"- Scripter: "SAMP_BLUE"Bo$$\n");
						strcat(info, ""WHITE"- Developer: "SAMP_BLUE"Bo$$\n");
						strcat(info, ""WHITE"- Lead Manager: "SAMP_BLUE"CxB,Guffin\n");
					...
					case 5:
					{
						...
						strcat(info, ""WHITE"Goto "SAMP_BLUE"COMING SOON "WHITE"and post your application in "SAMP_BLUE"Staff Application "WHITE"section.\n");
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if (0 <= GetPlayerTeam(playerid) < MAX_TEAMS)
	{
	    switch (GetVehicleModel(GetPlayerVehicleID(killerid)))
	    {
	   		...
	   		{
				if (IsPlayerInDynamicArea(playerid, gTeam[GetPlayerTeam(playerid)][teamBaseAreaId]))
				{
	    ...
	    foreach(new i : CarePackages) {
	        if(pCarePackage[i][CarePackageUsed] == true && pCarePackage[i][CarePackageLanded] == true) {
	            if(IsPlayerInRangeOfPoint(playerid,2,pCarePackage[i][CarePackageLand][0], pCarePackage[i][CarePackageLand][1], pCarePackage[i][CarePackageLand][2])) {
					KillTimer(CarePackageDespawnTimer[i]);
					...
					new string[128];
					format(string,sizeof(string),"{FFAF00}[{F3FF02}CARE PACKAGE{FFAF00}]{FFFFFF} You've picked up %s from care package.",CarePackageContains[pCarePackage[i][CarePackageStuff]][Lname]);
...
CMD:donate(playerid)
{
	...
	strcat(info, ""WHITE"1.Vishal lodu hai\n");
	...
	foreach(new i : CarePackages) {
	    if(objectid == pCarePackage[i][CarePackagePlane]) {
	        if(pCarePackage[i][PlaneMoveStage] == false) {
                ...
                pCarePackage[i][CarePackageParachute] = CreateDynamicObject(2903,X,Y,Z,0,0,random(360),pCarePackage[i][CarePackageWorld],pCarePackage[i][CarePackageInterior],-1,200,200);
	            ...
	            DestroyDynamicObject(pCarePackage[i][CarePackagePlane]);
            ...
            format(string,128,"{FFAF00}[{F3FF02}CARE PACKAGE{FFAF00}]\n{FFFFFF}%s",CarePackageContains[pCarePackage[i][CarePackageStuff]][Lname]);
            pCarePackage[i][CarePackageText] = Create3DTextLabel(string, 0x008080FF, pCarePackage[i][CarePackageLand][0], pCarePackage[i][CarePackageLand][1], pCarePackage[i][CarePackageLand][2]+1, 40.0, pCarePackage[i][CarePackageWorld], 0);
            CarePackageDespawnTimer[i] = SetTimerEx("DespawnCarePackage",CAREPACKRESPAWN,false,"i",i);
			DestroyDynamicObject(objectid);
...
CMD:acmds(playerid, params[])
{
	...
	{
		strcat(info, ""HOT_PINK" Administrator (Level 2):\n");
	...
	{
		strcat(info, ""HOT_PINK"Lead Administrator (Level 3):\n");
	...
	{
		strcat(info, ""HOT_PINK"Manager (Level 4):\n");
		...
		strcat(info, "/getteam, /spawnteam, /giveteamscore, /giveteamcash, /giveteamweapon, /giveteamhelmet, /giveteammask, /startfallout\n");
  		strcat(info, "Use `"GREEN"@"WHITE"' for admin level 4+ chat [eg. @ hello].\n\n");
	}
	if (pStats[playerid][userAdmin] >= 5 || IsPlayerAdmin(playerid))
	{
		strcat(info, ""HOT_PINK"Lead Manager (Level 5):\n");
...
CMD:mute(playerid, params[])
{
	...
	GameTextForPlayer(targetid, buf, 5000, 3);
  	new msg[128];
  	format(msg, sizeof(msg), "%s(%i) has been muted by admin %s(%i) for %i seconds [Reason: %s]", ReturnPlayerName(targetid), targetid, ReturnPlayerName(playerid), playerid, time, reason); // edit them
...
CMD:unmute(playerid, params[])
{
	...
	new msg[128];
  	format(msg, sizeof(msg), "%s(%i) has been unmuted by admin %s(%i).", ReturnPlayerName(targetid), targetid, ReturnPlayerName(playerid), playerid);
   	DCC_SendChannelMessage(logs, msg);
	return 1;
}
...
CMD:ban(playerid, params[])
{
	...
	if (! days)
	{
		...
		SendClientMessageToAll(COLOR_RED, buf);
 		new msg[128];
   		format(msg, sizeof(msg), "%s(%i) has been banned by admin %s(%i) [PERMANENT].", ReturnPlayerName(targetid), targetid, ReturnPlayerName(playerid), playerid); // edit them
   		DCC_SendChannelMessage(logs, msg);
	}
	else
	{
		...
		SendClientMessageToAll(COLOR_RED, buf);
 		new msg[128];
   		format(msg, sizeof(msg), "%s(%i) has been banned by admin %s(%i) for %i days [TEMPERORARY].", ReturnPlayerName(targetid), targetid, ReturnPlayerName(playerid), playerid, days); // edit them
   		DCC_SendChannelMessage(logs, msg);
	}
...
CMD:setlevel(playerid, params[])
{
    ...
    if (pStats[playerid][userAdmin] < level)
    {
        ...
        SendClientMessage(playerid, COLOR_DODGER_BLUE, buf);
       	new msg[128];
	  	format(msg, sizeof(msg), "```%s have promoted %s(%i) to admin level %i.```", ReturnPlayerName(playerid),ReturnPlayerName(targetid), targetid, level);
	   	DCC_SendChannelMessage(logs, msg);
    }
    else if (pStats[playerid][userAdmin] > level)
    {
		...
		SendClientMessage(targetid, COLOR_DODGER_BLUE, buf);
		format(buf, sizeof(buf), "```%s have demoted %s(%i) to admin level %i.```",ReturnPlayerName(playerid), ReturnPlayerName(targetid), targetid, level);
        SendClientMessage(playerid, COLOR_DODGER_BLUE, buf);
       	new msg[128];
	  	format(msg, sizeof(msg), "%s have demoted %s(%i) to admin level %i.",ReturnPlayerName(playerid), ReturnPlayerName(targetid), targetid, level);
	   	DCC_SendChannelMessage(logs, msg);
    }
	PlayerPlaySound(targetid, 1057, 0.0, 0.0, 0.0);
	PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
...
CMD:setpremium(playerid, params[])
{
    ...
    if (! pStats[targetid][userPremium] && set)
    {
        ...
        SendClientMessage(playerid, COLOR_DODGER_BLUE, buf);
       	new msg[128];
	  	format(msg, sizeof(msg), "%s have set %s(%i) to a premium user.",ReturnPlayerName(playerid), ReturnPlayerName(targetid), targetid);
	   	DCC_SendChannelMessage(logs, msg);
    }
    else
    {
        ...
        SendClientMessage(playerid, COLOR_DODGER_BLUE, buf);
       	new msg[128];
	  	format(msg, sizeof(msg), "%s have removed %s(%i)'s premium eligablity.", ReturnPlayerName(playerid),ReturnPlayerName(targetid), targetid);
	   	DCC_SendChannelMessage(logs, msg);
    }
	PlayerPlaySound(targetid, 1057, 0.0, 0.0, 0.0);
	PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
	    ...
	    if (pStats[i][userAdmin] || IsPlayerAdmin(i))
	    {
			...
			else
			{
			    switch (pStats[i][userAdmin])
			    {
			        ...
			        case 3: rank = "Lead Administrator";
			        case 4: rank = "Manager";
			        case 5: rank = "Lead Manager";
...
CMD:report(playerid, params[])
{
	...
	new msg[128];
  	format(msg, sizeof(msg), "REPORT: %s(%i) has reported against %s(%i), type /reports In-Game to check it.", ReturnPlayerName(playerid), playerid, ReturnPlayerName(targetid), targetid);
   	DCC_SendChannelMessage(logs, msg);
	foreach (new i : Player)
	{
		if (pStats[i][userAdmin] || IsPlayerAdmin(i))
...
CMD:changename(playerid, params[])
{
	...
	new msg[128];
	format(msg, sizeof(msg), "%s have changed your username from '%s' to '%s'.", ReturnPlayerName(playerid),ReturnPlayerName(playerid), name);
	DCC_SendChannelMessage(logs, msg);
...
CMD:stats(playerid, params[])
{
	...
	else
	{
	    switch (pStats[targetid][userAdmin])
	    {
	        ...
	        case 3: admin_rank = "Lead Administrator";
	        case 4: admin_rank = "Manager";
		    case 5: admin_rank = "Lead Manager";
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    for (new i; i < MAX_TEAMS; i++)
	{
		if (gTeam[i][teamBaseAreaId] == areaid)
  		{
  		    new text[45];
  		    strcat(text, "~y~~h~");
...
forward ShowUpdates(playerid);
public ShowUpdates(playerid)
{
    new BigString20[2000];
    strcat(BigString20, "{FFFF00}[1] :All commands logs will be recorded in discord\n\n");
    strcat(BigString20, "{FFFF00}[2] :New command /credits \n\n");
    ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{00FF00}Server {33CCFF}Updates ", BigString20, "Watched","Close");
    return 1;
}
...
CMD:credits(playerid)
{
    new info[1024];
	strcat(info, ""WHITE"Server Owner: "SAMP_BLUE"Bo$$\n\n");
	strcat(info, "Please visit our discord server "SAMP_BLUE"COMING SOON"WHITE".\n\n");
	strcat(info, ""LIME"Credits:\n");
	strcat(info, ""WHITE"Lead Managers: "SAMP_BLUE"- IndianMan,SmeXy,ItzFlawless_,Omega,CxB,Smokin,Venom\n");
	strcat(info, ""WHITE"Special Thanks to\nKalcor\nGamerX\nProtector\n\n");
	strcat(info,ReturnPlayerName(playerid));
	strcat(info, ""WHITE",For playing here!\n\n");
 	ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "Server Credits:", info, "Close", "");
	return 1;
}
CMD:startfallout(playerid,params[])
{
	...
	if(pStats[playerid][userAdmin] >= 4) return SendClientMessage(playerid,Red,"You need to be Admin to use that command!"); //If the player that tries to do that command is not admin we send him a warning
...
CMD:afk(playerid,params[])
{
	...
	SetPlayerPos(playerid,2166.8035,1265.6958,895.9060);
	SetPlayerInterior(playerid,0);
	SendClientMessage(playerid,COLOR_RED,"Use /kill to go back");
	GameTextForPlayer(playerid, "~r~Succesfully Teleported to AFK arena~r~ !", 5000, 5);
	return 1;
}
